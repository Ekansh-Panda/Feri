...
# ─────────────────────────────────────────────────────────────────────────────
# ComputerControl — class-based API required by the spec.
# ─────────────────────────────────────────────────────────────────────────────
...
class ComputerControl:
    """
    Thin facade over the legacy module-level helpers plus the small set of
    primitives (open_app, lock_screen, shutdown …) that did not exist there.
    """

    SCREENSHOT_DIR = _Path.home() / ".jarvis" / "screenshots"
    SCREENSHOT_DIR.mkdir(parents=True, exist_ok=True)

    # ── App launching / i3 workspace ────────────────────────────────────────

    def open_app(self, app: str) -> str:
        return _i3_exec(app)

    def focus_window(self, direction: str) -> bool:
        return _i3_msg_ok(f"focus {direction}")

    def move_window(self, direction: str) -> bool:
        return _i3_msg_ok(f"move {direction}")

    def split_h(self) -> bool:
        return _i3_msg_ok("split h")

    def split_v(self) -> bool:
        return _i3_msg_ok("split v")

    def set_layout(self, layout: str) -> bool:
        return _i3_msg_ok(f"layout {layout}")

    def workspace(self, num: int | str) -> bool:
        return _i3_msg_ok(f"workspace {num}")

    def move_to_workspace(self, num: int | str) -> bool:
        return _i3_msg_ok(f"move container to workspace {num}")

    def kill_focused(self) -> bool:
        return _i3_msg_ok("kill")

    def scratchpad(self) -> bool:
        return _i3_msg_ok("move scratchpad")

    def fullscreen_toggle(self) -> bool:
        return _i3_msg_ok("fullscreen toggle")
    ...
    def click(self, x: int | None = None, y: int | None = None,
              button: int = 1) -> str:
        return _click(x, y, {1: "left", 2: "middle", 3: "right"}.get(button, "left"))

    # ── Screenshot / clipboard ──────────────────────────────────────────────

    def screenshot(self, region: str = "full") -> _Path | None:
        out = self.SCREENSHOT_DIR / f"shot_{int(_time.time())}.png"
        if region == "full":
            r = _subprocess.run(["scrot", str(out)], capture_output=True, timeout=15)
        else:
            r = _subprocess.run(["scrot", "-a", region, str(out)], capture_output=True, timeout=15)
    ...
    def suspend(self) -> bool:
        return _power_action("suspend")

    def hibernate(self) -> bool:
        return _power_action("hibernate")

    def lock_screen(self) -> bool:
        """i3lock if available, else loginctl."""
        if _shutil.which("i3lock"):
            try:
                _subprocess.Popen(["i3lock"])
                return True
            except Exception:
                return False
        if _shutil.which("loginctl"):
            try:
                _subprocess.run(["loginctl", "lock-session"], capture_output=True, timeout=5)
                return True
            except Exception:
                return False
        return False
...
# ── Module-level helpers used by ComputerControl ─────────────────────────────

def _i3_msg_ok(*args: str) -> bool:
    if not _shutil.which("i3-msg"):
        return False
    try:
        r = _subprocess.run(
            ["i3-msg", *args], capture_output=True, timeout=5,
        )
        return r.returncode == 0 and b'"success":true' in (r.stdout or b"")
    except Exception:
        return False


def _i3_exec(command: str) -> str:
    if not _shutil.which("i3-msg"):
        return "i3-msg not installed"
    try:
        r = _subprocess.run(
            ["i3-msg", f"exec {command}"], capture_output=True, text=True, timeout=10,
        )
        return f"OK: exec {command}" if r.returncode == 0 else f"Failed: {r.stderr}"
    except Exception as e:
        return f"Failed: {e}"


def _power_action(verb: str) -> bool:
    if not _shutil.which("systemctl"):
        return False
    try:
        r = _subprocess.run(
            ["systemctl", verb], capture_output=True, timeout=8,
        )
        return r.returncode == 0
    except Exception:
        return False