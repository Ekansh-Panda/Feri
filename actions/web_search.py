...
def _read_url_text(url: str, max_chars: int = 12_000, timeout: int = 12) -> str:
    """Fetch a URL and extract readable text. Returns '' on failure."""
    try:
        import requests
...
class WebSearch:
    """
    High-level facade that exposes the underlying Gemini + DDG plumbing as
    instance methods so callers (agents, deep research, etc.) can use them
    without re-implementing quota/cooldown handling.
    """
    ...
    def news_search(self, query: str) -> list[dict]:
        """DDG news search."""
        return _ddg_news(query, max_results=10)

    def grounded_search(self, query: str) -> str:
        """Gemini search with google_search grounding tool."""
        return _gemini_search(query)

    def fetch_page(self, url: str, max_chars: int = 12_000) -> str:
        """Read a URL and return readable text."""
        return _read_url_text(url, max_chars=max_chars)
    ...
    def deep_research(
        ...
        """Run a multi-iteration research loop and write a markdown report."""
        from actions.deep_research import DeepResearch
        dr = DeepResearch(
            output_dir=output_dir,
            max_iterations=max_iterations,
            per_iter_timeout=self.timeout,
        )
        return dr.start_research(topic)
