...
# ─────────────────────────────────────────────────────────────────────────────
# WiFi, Bluetooth, power profile — added for the class-based API surface.
# Existing volume/brightness routines above remain untouched.
# ─────────────────────────────────────────────────────────────────────────────
...
def _wifi_interface_linux() -> str:
    if shutil.which("iwgetid"):
        try:
            ...
            pass
    for candidate in ("/sys/class/net/wlan0", "/sys/class/net/wlp3s0"):
...
def wifi_status() -> dict:
    """Returns current WiFi SSID, signal, interface or None."""
    if _OS == "Linux":
        if shutil.which("iwgetid"):
            try:
                r = subprocess.run(
                ...
                return {"ssid": ssid, "interface": _wifi_interface_linux(), "connected": bool(ssid)}
            except Exception:
                pass
        return {"ssid": "", "interface": _wifi_interface_linux(), "connected": False}
    if _OS == "Darwin":
        iface = "en0"
        ssid = ""
        if shutil.which("ipconfig"):
            try:
                ...
                pass
        return {"ssid": ssid, "interface": iface, "connected": bool(ssid)}
    return {"ssid": "", "interface": "", "connected": False}


def wifi_connect(ssid: str, password: str | None = None) -> bool:
    """Connect to a WiFi network (nmcli on Linux, networksetup on macOS)."""
    if _OS == "Linux" and shutil.which("nmcli"):
        ...
        try:
            ...
            return False
    if _OS == "Darwin" and shutil.which("networksetup"):
        iface = "en0"
...
def wifi_disconnect() -> bool:
    if _OS == "Linux" and shutil.which("nmcli"):
        try:
            r = subprocess.run(
                ["nmcli", "networking", "off"], capture_output=True, text=True, timeout=15
            )
            return r.returncode == 0
        except Exception:
            return False
...
def wifi_list() -> list[dict]:
    """Return a list of nearby WiFi networks."""
    networks: list[dict] = []
    if _OS == "Linux" and shutil.which("nmcli"):
        try:
            r = subprocess.run(
            ...
            pass
    if _OS == "Darwin" and shutil.which("airport"):
        try:
            r = subprocess.run(
...
def bluetooth_status() -> dict:
    """Bluetooth adapter + connected devices."""
    if _OS == "Linux" and shutil.which("bluetoothctl"):
        try:
            r = subprocess.run(
                ["bluetoothctl", "show"],
                capture_output=True, text=True, timeout=5,
            )
            powered = "Powered: yes" in r.stdout
            return {"powered": powered, "raw": r.stdout}
        except Exception:
            return {"powered": False, "raw": ""}
    return {"powered": False, "raw": ""}


def bluetooth_toggle() -> bool:
    if _OS == "Linux" and shutil.which("bluetoothctl"):
        status = bluetooth_status()
        new = "off" if status.get("powered") else "on"
        try:
            r = subprocess.run(
                ["bluetoothctl", "power", new],
...
_VALID_PROFILES = {"performance", "balanced", "power-saver"}


def set_power_profile(profile: str) -> bool:
    """Switch power profile (Linux power-profiles-daemon or TLP fallback)."""
    profile = (profile or "").lower().strip()
    if profile not in _VALID_PROFILES:
        return False
    if _OS == "Linux" and shutil.which("powerprofilesctl"):
        try:
            r = subprocess.run(
            ...
            pass
    # Fallback: cpupower governor
    if _OS == "Linux" and shutil.which("cpupower"):
        gov = {
...
class ComputerSettings:
    """Class API required by the spec — thin wrapper over the legacy functions."""
