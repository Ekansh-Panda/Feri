...
# ─────────────────────────────────────────────────────────────────────────────
# i3 workspace management — DesktopControl class API.
# Sits alongside the existing wallpaper/folder logic above.
# ─────────────────────────────────────────────────────────────────────────────
...
def _i3_msg(*args: str, timeout: int = 5) -> tuple[int, str, str]:
    if not _shutil.which("i3-msg"):
        return (-1, "", "i3-msg not installed")
    try:
        r = _subprocess.run(
            ["i3-msg", *args],
...
def _i3_msg_json(*args: str, timeout: int = 5) -> _Any:
    code, out, err = _i3_msg(*args, timeout=timeout)
...
class DesktopControl:
    """i3 workspace, layout, focus and resize control."""

    def __init__(self) -> None:
        self._is_i3 = _shutil.which("i3-msg") is not None

    def get_workspaces(self) -> list[dict]:
        return _i3_msg_json("-t", "get_workspaces") or []

    def get_tree(self) -> _Any:
        return _i3_msg_json("-t", "get_tree")

    def get_outputs(self) -> list[dict]:
        return _i3_msg_json("-t", "get_outputs") or []

    def go_to_workspace(self, name: str | int) -> bool:
        return _i3_msg(f"workspace {name}")[0] == 0

    def move_to_workspace(self, name: str | int) -> bool:
        return _i3_msg(f"move container to workspace {name}")[0] == 0

    def set_layout(self, layout: str) -> bool:
        return _i3_msg(f"layout {layout}")[0] == 0

    def toggle_floating(self) -> bool:
        return _i3_msg("floating toggle")[0] == 0

    def toggle_fullscreen(self) -> bool:
        return _i3_msg("fullscreen toggle")[0] == 0

    def focus_direction(self, direction: str) -> bool:
        if direction not in ("left", "right", "up", "down"):
            return False
        return _i3_msg(f"focus {direction}")[0] == 0

    def resize(self, direction: str, amount: int = 10) -> bool:
        if direction not in ("left", "right", "up", "down",
                              "width", "height"):
            return False
        return _i3_msg(f"resize {direction} {int(amount)} px")[0] == 0

    # Convenience aliases
    def workspace(self, num: int | str) -> bool:
        return self.go_to_workspace(num)

    def move_window(self, num: int | str) -> bool:
        return self.move_to_workspace(num)

    def split_h(self) -> bool:
        return self.set_layout("splith")

    def split_v(self) -> bool:
        return self.set_layout("splitv")

    def kill_focused(self) -> bool:
        return _i3_msg("kill")[0] == 0

    def scratchpad(self) -> bool:
        return _i3_msg("move scratchpad")[0] == 0

    def reload(self) -> bool:
        return _i3_msg("reload")[0] == 0